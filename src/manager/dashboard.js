...
export class Dashboard{
    ...
    notification(stocks){
        ...
        for(var i in stocks){
            var a = stocks[i];
            if((parseFloat(a.percentage || "0.00%") <= limitPercentage || a.no_of_days <= 30) && (a.discount+"").length == 0 && (a.ref_id == '' || a.ref_id == null)) total++;
            else if((parseFloat(a.percentage || "0.00%") != limitPercentage && a.no_of_days <= 30) && (a.discount+"").length == 0 && (a.ref_id == '' || a.ref_id == null)) total++;
            else if((a.no_of_days <= 2) && (a.discount+"").length > 0) total++;
        }
        return total;
    }
